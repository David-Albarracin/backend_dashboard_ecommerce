...
package pro.ddsr.backend_dashboard_ecommerce.employee.web;
...
import pro.ddsr.backend_dashboard_ecommerce.employee.domain.service.EmployeeService;
...
public class EmployeeController {
    ...
    @GetMapping("/byorders")
    // @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<EmployeesWithOrdersProjection>> viewEmployeesWithOrders(){
        List<EmployeesWithOrdersProjection> listEmployee  = employeeService.findAllEmployeesWithOrders();
        if (listEmployee.isEmpty()){
            return ResponseEntity.noContent().build();
        }
        return ResponseEntity.ok(listEmployee);
